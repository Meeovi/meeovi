...
const fetchCategories = async () => {
  try {
    const response = await $fetch(`${config.public.vendureUrl}/V1/categories`, {
      method: 'GET',
      headers: {
        'Content-Type': 'application/json',
        // If your API requires authentication for this endpoint, include the token
        'Authorization': `Bearer ${config.public.vendureApiToken}`
      }
    });
...
const savePersonalization = async () => {
  ...
  try {
    const response = await $fetch(`${config.public.vendureUrl}/V1/customers/me`, {
      method: 'PUT',
      headers: {
        'Content-Type': 'application/json',
        'Authorization': `Bearer ${config.public.vendureApiToken}` // You need to obtain and store the customer token
      },
      body: JSON.stringify({
        customer: {
          custom_attributes: [
