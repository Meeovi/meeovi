<template>
    <div>
      <v-col cols="12">
        <v-toolbar title="OTHER LEVELS" density="comfortable" color="transparent">
        </v-toolbar>
        <v-sheet class="mx-auto">
          <v-slide-group v-model="model" class="pa-4" selected-class="bg-success" show-arrows>
            <v-slide-group-item v-for="facetItem in levelsOnly" :key="facetItem.id || facetItem.name"
  ...
  const {
    ...
    // Client-side filter: only options whose `type` contains 'Levels' (trim whitespace)
    const levelsOnly = computed(() => {
      ...
      return list.filter(item => {
        const t = item?.type
        if (!t) return false
        if (typeof t === 'string') return t.trim() === 'Level'
